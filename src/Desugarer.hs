...
import Parser (NodeWithMetaData, Node (Variable, Function, Call, VariableDef, IntNode, IfStatement, Closure, EvalClosure, BuiltIn, RunBuiltIn, Block, RecursiveFunc, Infixes))
...
desugarer (context, node) = case node of 
    ...
    RecursiveFunc args body -> 
        desugarer (context, Call (context, Variable "Y'") [(context, Function ("rec":args) body)]) 
    Infixes startNode infixes -> 
        desugarer $ foldl 
            (\intermediate (operator, operand)-> 
                (context, Call operator [intermediate, operand])) 
            startNode infixes
